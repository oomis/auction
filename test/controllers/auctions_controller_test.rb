...
class AuctionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create auction" do
    assert_difference('Auction.count') do
      post auctions_url, params: { auction: { desc: @auction.desc, end_date: @auction.end_date, image: @auction.image, starting_price: @auction.starting_price, title: @auction.title, user_id: @auction.user_id } }
    end
  ...
  test "should update auction" do
    patch auction_url(@auction), params: { auction: { desc: @auction.desc, end_date: @auction.end_date, image: @auction.image, starting_price: @auction.starting_price, title: @auction.title, user_id: @auction.user_id } }
